...
{
char *s="TIMESLAUGHTER CHAT";
char *d=screen+7*320+20;
...
DrawStringSP(font5,screen+13*320+296,(unsigned char *)t,320);
...
int h;
unsigned size,bytes;
if (_dos_open("c:\\kali95\\kali95.ini",O_TEXT | O_RDONLY,&h)) strcpy(s,"Not found.");
 else
 {
...
netplayers=new netplayer(myaddress,idp.name,0,0,ipxmaster);
  ...
  while (kbhit())
    ...
    //normal keys----------
      ...
      if (key==13) //enter, do challenge
        {
         if (sp!=netplayers) //if not ourself
   ...
   while (np)
     {
       if (np->schallenge&1 && !(uu&63) )
         { actp.type=4; actp.data=np->schallenge; SendIPXPacketDest((char *)&actp,sizeof(actionpacket),(char *)&np->a.nodeadd);
         } //send the challenge
       np->SendText();
       np=np->next;
     }
   ...
   switch (ipd[0]) //packet type
   {
    case 0: //id
     ...
     {    //ADD it!
       ...
       if (!n->master)  sprintf(cmsg,"**%s joined TS Chat**",n->name);
    ...
    case 3:  //quit
     ...
     {
      if (!n->master)  sprintf(cmsg,"**%s quit TS Chat**",n->name);
