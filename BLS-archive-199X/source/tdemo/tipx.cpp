...
#define IPXCHATSOCKET 0x6100
#define IPXGAMESOCKET 0x6101
...
#define FIRSTSENDECB 1
#define  LASTSENDECB 19
#define FIRSTRECVECB 20
#define  LASTRECVECB 39
...
{
int i;    
for (i=FIRSTSENDECB; i<=LASTRECVECB; i++)
...
if (i) //if we found a good packet 
 {
  //return the bitch
  if (ipxmode)  //if game, then call handler
  {
    pr = reverse(ip[i].ipx.length) - sizeof(IPXHEADER) -4 ;
    memcpy (prbuf, ip[i].data, pr); //copy packet to packet recv buffer
    PacketIPXFunc();
    ipxlisten(i*sizeof(ipxpacket));
  }  
