                ...
                .DATA
                EXTERN _PITCH:DWORD,_SCREENX:DWORD,_SCREENY:DWORD
...
@@NOEXTRA2:     rep     movsd
                ...
                ;Transparent Run
@@TCOLOR:
                and     al,7Fh          ;mask off upper bits
                add     edi,eax
                sub     ebx,eax
                jg      @@VLOOP
...
BMONFLIPX       PROC          NEAR
        ARG @@SRC:DWORD,@@DEST:DWORD,@@LSW:DWORD,@@O:DWORD
                ...
                movzx   ebx,cl          ;X
                mov     edx,ecx         ;X => dl HVALUE  V

                add     edi,ebx         ;;Go to right of scan line
                xor     eax,eax
...
;@@NOXFLIP:
        ...
        xor     ecx,ecx ;cl=XST  ch=XLEN
        mov     ch,[esi+1]
        xor     edx,edx   ;dl=YST  dh=YLEN
        mov     dh,[esi+2]

        cmp     [@@X],0
        jge     @@NOXL
        inc     bl
        sub     cl,[byte ptr @@X]  ;xst=0-x
