...
{
 return addobject(new object(&odf[objnum],series,tx,ty,tz,td,this));
}
...
if (od->loaded)
  ...
  {active=1; csnum=s;} //store series num until it gets loaded
 ...
 if (!d)
 {
  for (; j>0; j--)
  {
   if ((!i->exclusive && i->layer<=layer) || (i->exclusive && i->layer==layer))
    DrawImage(od->id[i->index],screen,tx+i->dispx,ty+i->dispy,i->orient);
   i++;
  }
 } else
 {
  for (; j>0; j--)
  {
   if ((!i->exclusive && i->layer<=layer) || (i->exclusive && i->layer==layer))
   {
    ...
    DrawImage(iptr,screen,tx-i->dispx-iptr->xw,ty+i->dispy,0x2^i->orient);
   }
   i++;
  }
 }  
...
{ //create an instance of the object
 spawn(t->onum ? t->onum : onum,t->s,!d ? (x>>16)+t->pos.x : (x>>16)-t->pos.x,(y>>16)+t->pos.y,(z>>16)+(t->pos.z),
       (t->pos.d) ? d^1 : d);
...
int (object::* object::extrado[])(byte *)=
 {
