...
void __cdecl MIXASM(signed short *,signed char *,unsigned int,signed short *);
void __cdecl COPYASM(signed short *,signed char *,unsigned int,signed short *);
void __cdecl SILENCEASM(signed short *,unsigned int);
...
#define BLOCK_LENGTH    1024   /* Length of digitized sound output block     */
#define VOICES          8     /* Number of available simultaneous voices    */
#define VOLUMES         8    /* Number of volume levels for sound output   */
      ...
      { /* Eight bit */
        if (autoinit)
          { /* Eight bit auto-initialized:  SBPro and up (DSP 2.00+)        */
            ...
            write_dsp(193);             /*  = 256 - (1000000 / rate)        */
...
static void mix_voice(int voicenum)
  {
    ...
    signed short int *volume_lookup;
    int chunklength;
    int destindex;
...
//     (signed int *)(&((*volume_table)[(unsigned char)((((sound_volume/256.0) * voice[voicenum].volume) * (VOLUMES/256.0)))]));
       (signed short int *)(&((*volume_table)[sound_volume* voice[voicenum].volume/256 / (256/VOLUMES)]));
...
//          mixingblock[destindex++] += volume_lookup[(unsigned char)(*(sourceptr++))];
                ...
                {
                 if (donefirstmix)
                        MIXASM(&mixingblock[destindex],sourceptr,chunklength,volume_lookup);
                  else
                        {
                         COPYASM(&mixingblock[destindex],sourceptr,chunklength,volume_lookup);
                         if (BLOCK_LENGTH>chunklength)
                          SILENCEASM(&mixingblock[destindex+chunklength],(BLOCK_LENGTH-chunklength)*2);

                         donefirstmix++;
                        }
        
                 destindex+=chunklength;
                 sourceptr+=chunklength;
                }
...
void mix_voices(void)
  {
         ...
         if (!voicecount)
         {
          if (!wassilenced) {SILENCEASM(mixingblock,BLOCK_LENGTH*2); wassilenced=1; copysound=1;}
           else copysound=0;
         }
                else
         {
          wassilenced=0; copysound=1;
         for (i=0; i < VOICES; i++)
                if (inuse[i])
                  mix_voice(i);
    }
