...
DrawString(22,1,"Nogginknockers Installer Version 1.01");
...
for (i=0; i<dptr; i++)
 if (dletter[i]==sourcedrive) //find source dptr
 {
  ...
  if (_dos_findfirst("\\*.*",_A_VOLID,&ff) || stricmp(ff.name,"TS") || _dos_open("\\noggin.cow",0,&vh)) //find all labels
   {
     cdcheck++;
     OpenBox(12,10,54,2,0x4f);
...
DrawCenteredString(40,5,"The installation requires approximately 5MB of HD space.");
...
char s[50];
if (highlight<dptr)
 if (!dcdrom[highlight]) sprintf(s,"Install Nogginknockers 2 to drive %c:",dletter[highlight]+'A'-1);
   else strcpy(s,"");
else strcpy(s,"Exit without installing Nogginknockers 2.");
...
sprintf(installpath,"%c:\\NOGGIN2",installdrive+'A'-1);
...
unlink("noggin.cfg");
spawnl(P_WAIT,"nogsetup.exe",NULL);
}
