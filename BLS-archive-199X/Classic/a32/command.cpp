...
int option_setinput(char *p)
{
 ...
 return 1;
  ...
  /*
int option_definekey(char *p)
{
 int num;
 int ul,u,ur,l,r,dl,d,dr,b0,b1;
 sscanf(p,"%d %d %d %d %d %d %d %d %d %d %d",
  &num,&ul,&u,&ur,&l,&r,&dl,&d,&dr,&b0,&b1);

 num--;
 if (num<0 || num>1)
  {msg.error("Keyboard num out of range"); return 0;}
 keymap *k=&cfg->ids.km[num];
 k->ul=ul; k->u=u; k->ur=ur;
 k->l=l; k->r=r;
 k->dl=dl; k->d=d; k->dr=dr;
 k->button[0]=b0; k->button[1]=b1;
 return 1;
}   */
...
optiondef commanddefs[]=
{
 ...
 #ifndef ANIMATOR
 {"newosp", cmd_newosp, 0,0,                "Create blank objectspace"},
 {"startgame", cmd_startgame, 0,"<bgname>", "Loads <bgname> and all default objs"},
 {"loadbg", cmd_loadbg, 0,"<bgname>",       "Loads bg <bgname>"},
 {"load", cmd_loadobj, 0,"<objname> (X) (Y)",   "Loads object <objname> at X,Y"},
 {"maximize", cmd_maximize, 0,0,   "Maximize objectspace"},
 #endif

 #ifdef WIN95
 {"connect",cmd_connect,0, "<type> (addr) (port)", "Connect to network server"},
 {"server", cmd_server,0, "<type> (port)", "Start network server"},
 #endif
 {0,0,0,0,0}
};
