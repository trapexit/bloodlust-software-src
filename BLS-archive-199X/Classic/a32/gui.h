...
class GUIhexedit:public GUIintedit
{
 public:
 GUIhexedit(GUIrect *p,char *pmpt,int x,int y,int xw,int num,int min,int max):
  GUIintedit(p,pmpt,x,y,xw,num,min,max) {};
 virtual void drawdata(char *dest,int x,int y,int xw);
};
...
class GUIscrollbar:public GUIrect
{
 ...
 void pageup() {scroll(-(height()/10));}
 void pagedown() {scroll(+(height()/10));}
