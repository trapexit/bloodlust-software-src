...
void volumefile::writeheader(int type,int size,char *filename)
{
 strcpy(hdr.key,dsl);
 hdr.type=type;
 hdr.size=size;
...
void volumefile::writefile(char *file)
{
 ...
 if (g.open(file))
   //if (vwerror) {sprintf(errstr,"opening file %s",file); cleanexit(-1);}
    //else
    {write(0,0,V_RAWDATA,file); return; }
...
// msg.printf(2,"rate=%d size=%d",rate,size);
 ...
 if (vdiagnose)  printf(".wav samplerate: %5d size: %6d ",rate,size);

 if (rate<SOUNDFREQ)
  {
   if (vdiagnose) printf("expanding to %d...",SOUNDFREQ);
   unsigned finalsize=(unsigned)(((float)size)*SOUNDFREQ/rate);
 ...
 if (rate>SOUNDFREQ)
  {
   if (vdiagnose)
   printf("contracting to %d...",SOUNDFREQ);
   unsigned finalsize=(unsigned)(((float)size)*SOUNDFREQ/rate);
   SOUND *d2=(SOUND *)malloc(finalsize+4);
   d2->soundsize=finalsize;
...
/* char *ptr[256];
 ...
 ptr=(void **)calloc(num,4);
 idx=(unsigned *)calloc(num,4);
 sizes=(unsigned *)calloc(num,4);

 for (int i=0; i<num; i++) //read all images
...
//set up header
writeheader(V_BLOCK,tsize+4+num*4,list);
...
//msg.printf(2,"ptr[0]=%p size[0]=%d idx[0]=%d",ptr[0],sizes[0],idx[0]);
