...
{
 if (!vesainstalled) msg.printf(2,"DOS graphics limited to 320x200x256.");
  else
   {
...
{
 if (!vesainstalled) p->set(0,256);
  else
  {
   //copy from color format to VESA format
   for (int i=0; i<256; i++)
    {
     palentries[i].red=p->c[i].r;
     palentries[i].green=p->c[i].g;
     palentries[i].blue=p->c[i].b;
     palentries[i].alpha=0;
    }
   SV_setPalette(0,256,palentries,-1);
  }
}
...
{
 //dos mode 13h copy
 if (!vesainstalled) {memcpy(video,screen,64000); return;}
...
void main(int argc,char *arg[])
{
 ...
 if (linear) //take care of linear buffer shit
  {
   ...
   msg.printf(1,"linear buffer: %X -> %X\n",info.PhysBasePtr,linearvideo);
  }
...
int setvideomode(int x,int y,int colordepth)
{
 ...
 if (x==320 && y==200 && colordepth==8)
  {
   mode13h();
   SCREENX=320; SCREENY=200; PITCH=SCREENX;
   initmouse();
   setmouseextent(SCREENX,SCREENY);
   linear=0;
   if (vesainstalled)
   ...
   msg.printf(2,"VGA mode initialized. %dx%dx%d",SCREENX,SCREENY,8);
   return 1;
  }
