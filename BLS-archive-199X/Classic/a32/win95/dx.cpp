...
void wm_socketerror(SOCKET t,int,int);
...
void setpalette(PALETTE *pal)
{
 ...
 for (int i=0; i<256; i++)
  {
   palentries[i].peRed=pal->c[i].r<<2;
   palentries[i].peGreen=pal->c[i].g<<2;
   palentries[i].peBlue=pal->c[i].b<<2;
...
void changeresolution(int xw,int yw)
{
 ...
 releasesurfaces();
 if (setddrawmode(xw,yw)) setddrawmode(SCREENX,SCREENY);
 createsurfaces();
...
void objectdef::convertdsounds()
{
 //convert sounds to directsounds
 dsounds=(struct IDirectSoundBuffer **)malloc(numsounds*4); //allocate pointer array
 for (int i=0; i<numsounds; i++)
   dsounds[i]=createdsoundbuffer(sounds[i]);
}
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance,
                    PSTR szCmdLine, int iCmdShow)
  {
   ...
   if (initgame()!=0)
    {
     cleanup();
     winprintf("Unable to initialize grunt");
     return -1;
    };
