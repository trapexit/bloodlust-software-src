...
while ( i->index!=0xFF )
{
 DrawImage(imagelist[i->index],screen,tx+i->dispx[td],ty+i->dispy,i->orient^(td<<1));
 if (bloodlevel>=2)
  DrawPain(i->index,tx+i->dispx[td],ty+i->dispy,i->orient^(td<<1));
...
/*
  char midfile[30];
  for (int i=0; dir[i]!='.'; i++) midfile[i]=dir[i];
  strcpy(midfile+i,".MID");
  int th;
  _dos_creat(midfile,0,&th);
  music=(MUSIC *)malloc(30000);
  _dos_read(vh,music,size,&bytes);
  _dos_write(th,music,size,&bytes);
  free(music);
  _dos_close(th);
  lseek(vh,oldpos,SEEK_SET);
*/
  load_music(vh,&music);
...
if (musicstat && awe32)
 {
  int oldpos=lseek(vh,0,SEEK_CUR);
  int awesize=filelength(vh)-oldpos;
  if (awesize>0) //if there is something at the end....
   {
  free_music(&music);
/*
//---------
