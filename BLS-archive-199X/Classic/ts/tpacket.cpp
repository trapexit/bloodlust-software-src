...
extern int cominstalled,ipxinstalled;
...
{
if (cominstalled)   //send packet through modem
{    
 for (;num>0; num--,d++)
   ioWriteByte(port,*d);
}
...
packetbusy=1;
while (ioReadStatus(port))
{
 x=ioReadByte(port); //get next byte of packet

 prbuf[pr]=x; 

 if (!pr)   //check to see if it is first packet
  {
     if ((x&0xF0)==0xF0) //if upper part set, its a correct type
            {packettype=x&0xF; pr++; } //store it
    // else  {packeterror++; pe[0]++;}  //its wrong, skip it
  } else
  {
    pr++; //store it
     
    while (pr>=psize[packettype]) //if we've got all the packet, as determined by the type
     {
      int pskip;  
      if (CheckCRC(prbuf,psize[packettype]))  //check the CRC of the packet
        { //okcrc
         if (packethandler[packettype])    packethandler[packettype](); //call da handler
         packetrecvs++; pskip=psize[packettype]; //we consumed all this packet
         packettime=uu;
        } else {pskip=1; packeterror++; pe[1]++; } //error. skip type byte

         //keep skipping till we get to a new packet
      for ( ; pskip<pr; pskip++)
       if ((prbuf[pskip]&0xF0)==0xF0) //see if its a packet      
        {
          memcpy(prbuf,&prbuf[pskip],pr-pskip); //scoot it all over;
          pr-=pskip; packettype=prbuf[0]&0xF;
          break;  
        }
      if (pr==pskip) {pr=0; break;}
     }    
